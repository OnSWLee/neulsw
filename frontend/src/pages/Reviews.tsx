...
function Reviews() {
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      ...
      const query = `*[_type == "blogPost" && postType == "review"] | order(publishedAt desc) [${start}...${end}] {
        _id,
        title,
        excerpt,
        author,
      ...
      const countQuery = `count(*[_type == "blogPost" && postType == "review"])`;

      const [posts, total] = await Promise.all([
        client.fetch<Review[]>(query),
      ...
      console.error("후기 데이터 가져오기 오류:", err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-cream-white">
      <div className="mx-auto max-w-6xl px-6 py-12">
        {/* 헤더 */}
        <div className="mb-12">
          <h1 className="text-4xl font-bold text-slate-900">Story</h1>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex items-center justify-center py-20">
            <div className="text-center text-slate-600">
              <p className="mb-2">아직 작성된 후기가 없습니다.</p>
              <p className="text-sm text-slate-500">
                관리자 페이지에서 후기를 작성해주세요.
              </p>
            </div>
          </div>
        ) : (
